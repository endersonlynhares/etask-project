...
const ContentTask = () => {
  ...
  const onSetFinishTask = (content, isFinished) =>{
    const tasksWithoutAlteration = tasks.filter(task => task.content !== content)
    const taskAltered = {
      content: content,
  ...
  return (
    <div className={styles.contentTask}>
      <form className={styles.taskArea} onSubmit={handleSubmit(OnSubmit)} >
        <textarea required {...register("task", {required: true})} />
        <button type="submit">
          <PlusCircle size={30} />
        </button>
      </form>
      ...
      <div className={styles.info}>
        ...
        <strong>
          Tarefas Finalizadas <span className={styles.bunge}> {countTaskFinished} de {countTask} </span>
        </strong>
      </div>
